...
def create_launch_string():
    ...
    #return "{}={} ! qtdemux ! vaapih264dec ! \
    #return "{}={} ! decodebin ! \
    #gvaclassify model={} device=CPU batch-size=1 ! queue ! \
    #gvaclassify model={} device=CPU batch-size=1 ! queue ! \
    #gvaclassify model={} batch-size=1 ! queue ! \
    #gvadetect model={} device=MYRIAD batch-size=1 ! \
    #gvapython module=/opt/intel/openvino_2021/data_processing/dl_streamer/samples/python/draw_face_attributes/yolov4_post.py ! \
    return "{}={} ! qtdemux ! vaapih264dec ! \
    videoconvert ! capsfilter caps=\"video/x-raw,format=BGRx\" ! \
    gvainference model={} device=MYRIAD ! queue ! \
    gvapython module=/opt/intel/openvino_2021/data_processing/dl_streamer/samples/python/draw_face_attributes/yolov4_post.py ! \
